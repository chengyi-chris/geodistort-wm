...
def distort_images(images: typing.Union[Image.Image, typing.List[Image.Image]],
    ...
    for img in images:
        ...
        if rba_delta is not None:
            import cv2
            # 轉換 PIL 到 NumPy (OpenCV 格式)
            ...
            # 正弦包絡線
            envelope = np.sin(2.0 * np.pi * rba_fx * xx) * np.sin(2.0 * np.pi * rba_fy * yy)

            # 生成平滑隨機位移
            nx = np.random.normal(loc=0.0, scale=1.0, size=(h, w)).astype(np.float32)
            ny = np.random.normal(loc=0.0, scale=1.0, size=(h, w)).astype(np.float32)
            nx = cv2.GaussianBlur(nx, (0, 0), sigmaX=1.0, sigmaY=1.0)
            ny = cv2.GaussianBlur(ny, (0, 0), sigmaX=1.0, sigmaY=1.0)

            # 正規化雜訊至約 [-1, 1] 並計算最終位移
            nx /= (np.max(np.abs(nx)) + 1e-8)
            ny /= (np.max(np.abs(ny)) + 1e-8)
            
            # noise_scale 這裡預設給 0.35 (StirMark 標準值)
            noise_scale = 0.35
            dx = rba_delta * envelope * (1.0 + noise_scale * nx)
            dy = rba_delta * envelope * (1.0 + noise_scale * ny)

            # 執行重映射
            map_x = (xx + dx).astype(np.float32)
            map_y = (yy + dy).astype(np.float32)
            
            # 注意：若為多通道圖片，cv2.remap 會自動處理
