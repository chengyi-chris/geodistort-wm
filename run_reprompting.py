...
for id, (target_prompt, attacker_prompt) in tqdm(enumerate(zip(target_prompts, attacker_prompts)), total=len(target_prompts), desc="Generating images"):
    ...
    with torch.no_grad():
        ...
        harmful_image_summary = {
            "run_id": id + 1,
            "log_message": f"(Harmful image) detection_success: {detection_successful}, bit accuracy: {results_harmful['bit_accuracy']:.5f}, p_value: {results_harmful['p_value']}. PRC value: {results_harmful['value']:.5f}, l1_dist: {-results_harmful['l1_dist']:.5f}"
        }
        ...
        # log
        if args.logger:
            logger.info(f"(Harmful image) detection_success: {detection_successful}, bit accuracy: {results_harmful['bit_accuracy']:.5f}, p_value: {results_harmful['p_value']}. PRC value: {results_harmful['value']:.5f}, l1_dist: {-results_harmful['l1_dist']:.5f}")
        else:
            print(f"(Harmful image) detection_success: {detection_successful}, bit accuracy: {results_harmful['bit_accuracy']:.5f}, p_value: {results_harmful['p_value']}. PRC value: {results_harmful['value']:.5f}, l1_dist: {-results_harmful['l1_dist']:.5f}")
