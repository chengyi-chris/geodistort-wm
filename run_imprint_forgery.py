...
parser.add_argument("--lr", type=float, default=1e-2) # default 1e-2
...
# diffpipe = imprint_utils.DiffPipeSD3(pipe_attacker, scheduler=inverse_scheduler, device=pipe_attacker.device)
...
prc_value = 0.0
if args.wm_type == "GS":
    value = results["bit_accuracy"]
elif args.wm_type == "TR":
    value = results["p_value"]
elif args.wm_type == "PRC":
    value = results["value"]
    prc_value = value
...
for step in tqdm.tqdm(range(args.steps)):
    ...
    if step % args.validation_steps == 0:
        ...
        prc_value = 0.0
        if args.wm_type == "GS":
            value = results_harmful["bit_accuracy"]
        elif args.wm_type == "TR":
            value = results_harmful["p_value"]
        elif args.wm_type == "PRC":
            value = results_harmful["value"]
        ...
        detection_successful = check_if_detection_successful(wm_type=args.wm_type,
                                                             threshold=detection_threshold,
                                                             value=value)
        # print(results['message_bits_str_recovered'])
